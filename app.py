...
if not GROQ_API_KEY:
    raise RuntimeError("GROQ_API_KEY not found. Check .env or environment variables")
...
for file in chat_files:
    ...
    if st.sidebar.button(label):
        ...
        st.session_state.chat = json.loads(file.read_text())
        st.session_state.conversation = [
            {"role": "system", "content": "You are an academic research assistant."}
        ] + st.session_state.chat
        st.rerun()
...
"""
    ...
    (CHAT_DIR / f"{st.session_state.chat_id}.json").write_text(
        json.dumps(st.session_state.chat, indent=2)
    )
